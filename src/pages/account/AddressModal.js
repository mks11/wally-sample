...
class AddressModal extends Component {
  ...
  handleMakeDefault() {
    ...
    this.userStore
      .makeDefaultAddress(this.state.address_id)
      .then((data) => {
        this.userStore.setUserData(data);
        this.userStore.hideAddressModal();
      })
  ...
  render() {
    ...
    return (
      <Modal
        isOpen={this.userStore.addressModal}
        className="modal-outline"
        onClosed={(e) => this.userStore.closeAddressModal()}
      >
        <div className="modal-header">
          <h2>{this.state.title}</h2>
          <button
            className="btn-icon btn-icon--close"
            onClick={(e) => this.userStore.hideAddressModal()}
          ></button>
        </div>
            ...
            <textarea
              ...
              placeholder={
                this.state.mode !== 'edit'
                  ? 'Add delivery instructions (e.g. “use callbox when you arrive”)'
                  : ''
              }
              onChange={(e) =>
                this.setState({ delivery_notes: e.target.value })
              }
                ...
                <FormGroup check className="my-2">
                  <Label check>
                    <Input
                      ...
                      onChange={(e) =>
                        this.setState({
                          preferred_address: !this.state.preferred_address,
                        })
                      }
                    />{' '}
            ...
            <ModalBody className="modal-body-bordertop">
              <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                <button
                  ...
                  className="btn btn-main my-3 white"
                  style={{ width: '40%' }}
                >
                  DELETE
                </button>
                ...
                <button
                  onClick={(e) => this.handleMakeDefault(e)}
                  className={
                    'btn btn-main active my-3' +
                    (this.state.default ? ' disabled' : '')
                  }
                  style={{ width: '40%' }}
                >
                  MAKE DEFAULT
          ...
          {this.state.deleteConfirmation ? (
            <ModalBody className="modal-body-bordertop">
              <button
                ...
                className="btn btn-main my-3 active"
              >
                CONFIRM
              </button>
              <span className="text-error text-center text-block">
