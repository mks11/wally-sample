...
class ProductWithPackaging extends Component {
  ...
  async componentDidMount() {
    ...
    const priceMultiplier = product.buy_by_packaging ? product.packaging_vol[0] : 1
    this.setState({priceMultiplier: priceMultiplier});

    const packagingType = product.buy_by_packaging ? product.packagings[0].type : null
  ...
  handleAddToCart = () => {
    ...
    logEvent({category:"Product", action:"AddToCart", value:this.state.qty, label:productStore.activeProductId})
    ...
    const finalUnitType =
      (product.buy_by_packaging && packagingType)
        ? 'packaging'
        : unit_type
    const packaging = product.packagings[0] ? product.packagings[0] : null
    const defaultPackagingId = packaging ? packaging.id : null
    const customPackaging = packagingType ? product.packagings.find(p => p.type === packagingType)._id : null
    ...
    if (quantityAddon > 0) {
      const addonProduct = product.add_ons.find(p => p.product_id === packagingAddon)
  ...
  render() {
    ...
    if (product.available_inventory[0]) shipMessage = `Sold by ${product.available_inventory[0].shop}, fulfilled by The Wally Shop`;
    if (product.fbw) shipMessage = "Sold by " + product.vendor + ", fulfilled by The Wally Shop."
    ...
    const inventory = product.available_inventory[0] ? product.available_inventory[0] : null
    const limitOptions = product.fbw && !product.out_of_stock && product.available
    let qtyOptions = []

    const incrementValue = (product.buy_by_packaging && packagingType) ? 1 : product.increment_size
    const minSize = (product.buy_by_packaging && packagingType) ? 1 : product.min_size
                ...
                (
                  <React.Fragment>
                    ...
                    <AmountGroup
                      ...
                      selected={product.packagings ? product.packagings[0].type : null}
            ...
            <Col>
              ...
              <div className="media media-xs">
                <div className="media-body">
                  ...
                  {
                    product.fbw && product.description && <div><span className="font-weight-bold">Description:</span> {product.description}</div>
                  }
                  {
                    product.fbw && product.instruction && <div><span className="font-weight-bold">Instructions:</span> {product.instruction}</div>
                  }
                </div>
              </div>
            </Col>
