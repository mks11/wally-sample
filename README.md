...
This repository constitutes The Wally Shop's web app.
...
- [The Wally Shop](#the-wally-shop)
  ...
  - [Project Structure](#project-structure)
    - [A deeper look at /src](#a-deeper-look-at-src)
...
- [Node Version Manager](https://github.com/nvm-sh/nvm) for managing node versions.
- [VSCode](https://code.visualstudio.com/)
  - NOTE - it's possible to use any modern IDE for writing code, but VSCode is recommended so editor plugins and settings
    can be shared among our developers to improve the consistency of our development efforts.

### Installation

1. Install git if not installed already.
2. Clone [the git repository](https://github.com/tamlim/the-wally-shop-frontend) after being granted access as a colaborator.
   1. run `git clone https://github.com/tamlim/the-wally-shop-frontend.git && cd the-wally-shop-frontend` in your directory of choice.
...
- `/public`: Files that are going to be used directly by the page template, `index.html` reside in this folder.
- `/server`: Mock server implementation for testing. Will be removed in a future version.
- `/src`: This directory contains all of the code used to build the website. src is a convention for “source code”.
- `.editorconfig`: This file is used to maintain consistency between different IDEs.
- `.env`: Environment variables used by `create-react-app`. Contrary to some conventional `.env` files, nothing sensitive (API keys, etc.) should be placed in this file since it's committed to git.
- `.eslintrc.json`: This is the config file for `eslint`, which is a JS linter used to catch static errors in the code as it's written.
- `.firebaserc`: One of the two firebase config files. Lists projects and deploy targets.
- `.gitignore`: This file tells git which files it should not track / not maintain a version history for.
- `.prettierignore`: This is a configuration file for Prettier. It tells Prettier which files to ignore when enforcing code style.
...
- `.stylelintrc.json`: This is a configuration file for Stylelint. Stylelint is a tool to help keep the formatting of CSS consistent. It works on vanilla CSS, CSS Modules, and `styled-components` JSS.
- `firebase.json`: The second firebase config files. Defines settings for the various firebase services. Only hosting in our case.
- `jsconfig.json`: Defines the application's base path for file imports so that relative paths can be used to import modules throughout the app.
...
### A deeper look at /src

These are the top level directories and files in the `/src` folder.
...
- `/api`: Application's API layer. Methods that make API calls to our backend or third-party APIs are stored here.
- `/aux`: Legacy webpage displayed to kickstarter backers.
- `/common`: This directory contains React components used throughout the project.
- `/forms`: Contains formik forms. Forms will be moved to the directories they're used in and this will be removed in a future version.
- `/hooks`: Contains custom React hooks.
- `/modals`: Various modal components. Gradually migrating away from these modals in favor of reuseable components that can be passed directly as children into a single modal component.
- `/pages`: Webpage components.
- `/services`: Reusable functions containing some business logic specific to our application that's used by functions provided by third party libraries.
- `/snackbars`: Customized implementations of snackbar components used for in-app notifications.
- `/stores`: MobX stores.
- `/styled-component-lib`: Reusable styled components.
- `/templates`: Reusable page templates. Implemented to make a slow migration to a static site generator framework less painful.
- `/config.js`: Contains API routes and determines the url of the backend (either development or production) to connect to based on the environment the app is running in.
- `/generate-sitemap.js`: JS used to generate a sitemap from a set of routes.
- `/index.js`: The main React application template.
- `mui-theme.js`: Contains the theme used by Material-UI
- `registerServiceWorker.js`: While currently unused, this file enables offline behaviors specified by the PWA spec.
- `robots-txt.config.js`: Configuration file for module used to generate a robots.txt file on every build.
- `routes.js`: React router configuration.
- `utils.js`: Commonly used utility methods.
